...
import styles from './report-card/styles';
...
function ReportCard({ids}: {ids: string[]}) {
  ...
  useEffect(() => {
    const fetchStudentsData = async () => {
      try {
        ...
        const studentIds = ids && ids.length > 0 ? ids : ["UCA2025019", "UCA2025011"];

        const response = await axios.get("/api/reports", { 
          params: { ids: studentIds, reportType, role } 
        });
